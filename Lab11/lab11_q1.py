...
def invert_binary_tree(LBT):
    invert_binary_tree_helper(LBT.root)
    return LBT


def invert_binary_tree_helper(curr_root):
    if curr_root.left is None or curr_root.right is None:
        return
    else:
        temp = curr_root.left
        curr_root.left = curr_root.right
        curr_root.right = temp
        invert_binary_tree_helper(curr_root.left)
        invert_binary_tree_helper(curr_root.right)
...
a = invert_binary_tree(binary_tree1)
for i in a:
    print(i, end=' ')
print()
