...
def is_bst_helper(curr_root):
    ...
    if curr_root.left is None and curr_root.right is None:
        return True, curr_root.item.value, curr_root.item.value
    elif curr_root.left is None:
        res = is_bst_helper(curr_root.right)
        if res[1] < curr_root.item.value and res[2] > curr_root.item.value:
            st = True
            return (st and res[0]), res[1], res[2]
        else:
            st = False
            a = min(curr_root.value, res[1], res[2])
            b = max(curr_root.value, res[1], res[2])
            return (st and res[0]), a, b

    elif curr_root.right is None:
        res = is_bst_helper(curr_root.left)
        if res[1] < curr_root.item.value and res[2] > curr_root.item.value:
            st = True
            return (st and res[0]), res[1], res[2]
        else:
